...
#         # harmonic_oscillator: 1/2 * m * omega^2 * x^2
#         omega = 1.0
#         electron_mass = 1.0  # assumed
#         V_confining = 0.5 * electron_mass * omega**2 * x**2
#         V_external = V_confining.copy()
        
#     elif potential_type == "infinite_well":
#         # Infinite well from a to b
#         a, b = -5, 5
#         large_value = 1e12
#         V_confining = np.zeros_like(x)
#         # Note: x is linspace(-5,5), so boundary points might catch this. 
#         # We rely on grid points to define the "inside".
#         V_confining[x < a] = large_value
#         V_confining[x > b] = large_value
#         V_external = np.zeros_like(x)  # External potential is usually zero inside well
...
#         # Fix the mask to be strictly inside to see the well
#         mask = (x > -3) & (x < 3) # Adjusted slightly so it's visible within -5,5 grid
#         V_confining[mask] = height_depth
#         V_external = V_confining.copy()
...
#         # Two separate wells
#         left_well_a, left_well_b = -4, -1.5
#         left_well_height_depth = -1
#         right_well_a, right_well_b = 1.5, 4
#         right_well_height_depth = -1
#         y_level = 0
        
#         V_confining = np.full_like(x, y_level)
#         V_confining[(x > left_well_a) & (x < left_well_b)] = left_well_height_depth
#         V_confining[(x > right_well_a) & (x < right_well_b)] = right_well_height_depth
#         V_external = V_confining.copy()
...
# ###############################################################################
# def train_ml_model(x, D2, num_electron, potential_type, n_training=20):
#     """
#     Train a linear model to predict density from potential.
#     Now specifically tailored to the potential type being tested.
#     """
#     print(f"  Training ML model for potential type: {potential_type}...")
...
#     # Generate training data: Base potential + noise
#     np.random.seed(42)
    
#     # Get the clean base potential shape
#     V_base, _ = get_potential(potential_type, x)
    
#     for i in range(n_training):
#         # 1. Start with the specific base potential
#         # 2. Add random noise (0.3 scale)
#         V_train = V_base + 0.3 * np.random.randn(len(x))
        
#         # Calculate resulting density
...
#     # Regularization could be added here if S has very small values
...
#         # Random numbers (normalized later)
...
#     - zero, ones, random, gaussian, ml
#     """
#     results = {}
    
#     # Define the initial guess types to test (added "random")
...
#         W = None
#         if use_ml:
#             W = train_ml_model(x, D2, num_electron, pot_type, n_training=20)
        
#         # 2. Get the actual potential for SCF
...
#             plt.plot(x, V_external, 'r--', linewidth=2, label='External potential')
#         plt.xlabel('x')
#         plt.ylabel('V(x)')
...
#         colors = {'zero': 'blue', 'ones': 'green', 'random': 'purple', 'gaussian': 'orange', 'ml': 'red'}
#         markers = {'zero': 'o', 'ones': 's', 'random': 'x', 'gaussian': '^', 'ml': 'D'}
...
#                         color=colors.get(guess_type, 'black'),
#                         marker=markers.get(guess_type, '.'),
#                         linewidth=2, 
#                         markersize=4, 
#                         label=f'{guess_type}',
...
#     width = 0.15
#     colors = {'zero': 'skyblue', 'ones': 'lightgreen', 'random': 'purple', 'gaussian': 'orange', 'ml': 'salmon'}
...
#     print("="*100)
#     header = f"{'Potential':<20}"
#     for gt in guess_types:
#         header += f" {gt.capitalize():<8}"
#     print(header)
#     print("─"*100)
...
def plot_summary(results):
    ...
    for pot in pot_types:
        ...
        for gt in guess_types:
            ...
            # Formatting: If improvement is positive, Greenish/Normal. If negative (downgrade), mark it.
