...
def get_potential(potential_type, x):
    ...
    Matches user's exact working code
    """
    if potential_type == "harmonic":
        ...
        # Infinite well from a to b (default: -1 to 1)
        a, b = -5, 5
        large_value = 1e12
        V_confining = np.zeros_like(x)
        V_confining[x < a] = large_value
        V_confining[x > b] = large_value
        V_external = np.zeros_like(x)  # External potential is zero
        
    elif potential_type == "finite_well":
        # Finite square well: height_depth inside [a,b], y_level outside
        height_depth = -1
        a, b = -5, 5
        y_level = 0
        ...
        # Step potential: y_lower for x < x_jump, y_upper for x >= x_jump
        x_jump = 0
        y_lower = 0
        y_upper = 5
        ...
        # Two separate wells at [left_well_a, left_well_b] and [right_well_a, right_well_b]
        left_well_a, left_well_b = -5, -2
        left_well_height_depth = -1
        right_well_a, right_well_b = 2, 5
...
def get_initial_guess(guess_type, x, V_external, W, num_electron):
    ...
    Parameters:
    - guess_type: "zero", "ones", "gaussian", "ml"
    - x: grid
...
def compare_potentials(potential_types, num_electron=17, use_ml=True):
    ...
    # Train ML model once (used for all potentials)
    if use_ml:
        W = train_ml_model(x, D2, num_electron, n_training=20)
        print("\n" + "="*70)
        print("NOTE: ML model trained ONCE and reused for all potentials")
        print("="*70)
    else:
        W = None
    
    # Define the initial guess types to test
    guess_types = ["zero", "ones", "gaussian", "ml"] if use_ml else ["zero", "ones", "gaussian"]
    ...
    for pot_type in potential_types:
        ...
        colors = {'zero': 'blue', 'ones': 'green', 'gaussian': 'orange', 'ml': 'red'}
        markers = {'zero': 'o', 'ones': 's', 'gaussian': '^', 'ml': 'D'}
...
def plot_summary(results):
    ...
    width = 0.2
    colors = {'zero': 'skyblue', 'ones': 'lightgreen', 'gaussian': 'orange', 'ml': 'salmon'}
    ...
    x_pos = np.arange(len(pot_types))
    width = 0.25
    colors_savings = {'ones': 'green', 'gaussian': 'orange', 'ml': 'red'}
    ...
    print("="*100)
    print(f"{'Potential':<20} {'Zero':<8} {'Ones':<8} {'Gaussian':<10} {'ML':<8} "
          f"{'Ones vs Zero':<15} {'Gauss vs Zero':<15} {'ML vs Zero':<15}")
    print("─"*100)
    ...
    for pot in pot_types:
        ...
        print(f"{pot:<20} {zero_i:<8} {ones_i:<8} {gauss_i:<10} {ml_i:<8} "
...
if __name__ == "__main__":
    ...
    # List of potentials to test (matching user's code)
    potential_types = [
        "harmonic",
        "infinite_well",
        "finite_well",
    ...
    print("Comparing 4 initial guesses: zero, ones, gaussian, ML")
