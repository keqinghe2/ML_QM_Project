...
def get_potential(potential_type, x):
    ...
    if potential_type == "harmonic":
        ...
        # Finite square well: depth = -5 inside [-1, 1], zero outside
        V_confining = np.zeros_like(x)
        V_confining[(x > -1) & (x < 1)] = -5.0
        V_external = V_confining.copy()
        
    elif potential_type == "step":
        # Step potential: 0 for x < 0, height 5 for x >= 0
        V_confining = np.zeros_like(x)
        V_confining[x >= 0] = 5.0
        V_external = V_confining.copy()
        
    elif potential_type == "double_well":
        # Double well potential (quartic)
        V_confining = 0.5 * x**4 - 4 * x**2
        V_external = V_confining.copy()
        
    elif potential_type == "double_square_well":
        # Double square well: two separate wells at [-2,-1] and [1,2], depth -1
        V_confining = np.zeros_like(x)
        V_confining[(x > -2) & (x < -1)] = -1.0
        V_confining[(x > 1) & (x < 2)] = -1.0
        V_external = V_confining.copy()
